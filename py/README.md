# CelestrakGpData Python SDK



The Python SDK for the CelestrakGpData API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Gpn()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/celestrak-gp-data-sdk/releases)) or
...
```python
client = CelestrakGpDataSDK.test(testopts, sdkopts)
```
...
| `CLASSIFICATION_TYPE` | `str` | Classification (U=Unclassified, C=Classified, S=Secret) |
| `ECCENTRICITY` | `float` | Orbital eccentricity |
| `ELEMENT_SET_NO` | `int` | Element set number |
| `EPHEMERIS_TYPE` | `int` | Ephemeris type |
| `EPOCH` | `str` | Epoch time of the orbital elements |
...
| `OBJECT_NAME` | `str` | Name of the space object |
...
```
py/
├── celestrakgpdata_sdk.py         -- Main SDK module
...
The main module (`celestrakgpdata_sdk`) exports the SDK class.
