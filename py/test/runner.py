# CelestrakGpData SDK test runner
...
class CelestrakGpDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CelestrakGpDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CelestrakGpDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CelestrakGpDataTestRunner.getenv("CELESTRAK_GP_DATA_TEST_LIVE")
        override = CelestrakGpDataTestRunner.getenv("CELESTRAK_GP_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CelestrakGpDataTestRunner.getenv(key)
        ...
        explain = CelestrakGpDataTestRunner.getenv("CELESTRAK_GP_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CelestrakGpDataTestRunner._test_control is not None:
            return CelestrakGpDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CelestrakGpDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CelestrakGpDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CelestrakGpDataTestRunner.load_test_control()
...
def load_env_local():
    CelestrakGpDataTestRunner.load_env_local()


def env_override(m):
    return CelestrakGpDataTestRunner.env_override(m)


def entity_data(v):
    return CelestrakGpDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CelestrakGpDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CelestrakGpDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CelestrakGpDataTestRunner.load_test_control()


def live_client_options():
    return CelestrakGpDataTestRunner.live_client_options()


def live_delay_ms():
    return CelestrakGpDataTestRunner.live_delay_ms()
