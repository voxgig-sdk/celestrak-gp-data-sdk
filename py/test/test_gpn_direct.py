...
class TestGpnDirect:
    ...
    def test_should_direct_list_gpn(self):
        setup = _gpn_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-gpn", "live" if setup["live"] else "unit")
        if _skip:
