...
boru-driven command-line client **and** interactive REPL for the CelestrakGpData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/celestrak-gp-data-cli)
make build

# 2. See usage (words, entities, env vars)
./celestrak-gp-data-cli --help
...
CELESTRAK_GP_DATA_BASE=https://api.example.com ./celestrak-gp-data-cli list gpn

# 6. No arguments -> interactive REPL
./celestrak-gp-data-cli
celestrak-gp-data> list gpn
celestrak-gp-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/celestrak-gp-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/celestrak-gp-data-cli list gpn
   ```
...
Run with no arguments to open a REPL (prompt `celestrak-gp-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./celestrak-gp-data-cli
celestrak-gp-data> list gpn
celestrak-gp-data> /help
celestrak-gp-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `gpn`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/celestrak-gp-data-cli`. |
...
(`./celestrak-gp-data-cli <expr>`) and interactively (the REPL), and expressions compose the
