// Command CelestrakGpData-cli is a CLI + REPL for the CelestrakGpData SDK,
...
const prompt = "celestrak-gp-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "gpn"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CELESTRAK_GP_DATA_APIKEY (api key), CELESTRAK_GP_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CELESTRAK_GP_DATA_APIKEY carries the API key and
	// CELESTRAK_GP_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CELESTRAK_GP_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CELESTRAK_GP_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCelestrakGpDataSDK(opts)
