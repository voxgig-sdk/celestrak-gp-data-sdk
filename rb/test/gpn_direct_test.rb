...
class GpnDirectTest < Minitest::Test
  def test_direct_list_gpn
    setup = gpn_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-gpn", setup[:live] ? "live" : "unit")
    if _should_skip
...
def gpn_direct_setup(mockres)
  ...
  if live
    ...
    client = CelestrakGpDataSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
