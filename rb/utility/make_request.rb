...
module CelestrakGpDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CelestrakGpDataResponse.new({})
    ...
    if fetch_err
      ...
      response = CelestrakGpDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CelestrakGpDataResponse.new(fetched)
    else
