...
class GpnEntity
  ...
  def make
    opts = @_entopts.dup
    GpnEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CelestrakGpDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Gpn, Hash] the current Gpn data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CelestrakGpDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Gpn fields)
  def match_get
  ...
  #
  # @param reqmatch [GpnListMatch, Hash, nil] match filter (any subset of
  #   Gpn fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Gpn>, Array] the matching Gpn items; raises CelestrakGpDataError on failure
