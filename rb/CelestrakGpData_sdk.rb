...
require_relative 'CelestrakGpData_types'
...
class CelestrakGpDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CelestrakGpDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CelestrakGpDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CelestrakGpDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CelestrakGpDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CelestrakGpDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CelestrakGpDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CelestrakGpDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CelestrakGpDataError => err
    ...
    if fetched.is_a?(Hash)
      status = CelestrakGpDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CelestrakGpDataError.new(
        "graphql_error", "CelestrakGpDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CelestrakGpDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
