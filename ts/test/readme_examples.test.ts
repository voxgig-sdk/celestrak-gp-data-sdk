...
//     (\`new CelestrakGpDataSDK(...)\` / \`CelestrakGpDataSDK.test(...)\` become
//     \`CelestrakGpDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CelestrakGpDataSDK'
...
const TEST_SEED = {"entity":{"gpn":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CelestrakGpDataSDK(...)\` and any
// \`CelestrakGpDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CelestrakGpDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CelestrakGpDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
