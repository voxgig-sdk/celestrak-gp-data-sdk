...
import (
	...
	omni "github.com/voxgig-sdk/celestrak-gp-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CelestrakGpDataError); is {
				out := map[string]any{
					"name":    "CelestrakGpDataError",
...
func omniCtx(arg any, client *sdk.CelestrakGpDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CelestrakGpDataSDK); is {
			client = live
