# CelestrakGpData Golang SDK



The Golang SDK for the CelestrakGpData API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/celestrak-gp-data-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/celestrak-gp-data-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/celestrak-gp-data-sdk/go=../celestrak-gp-data-sdk/go
```
...
func main() {
    ...
    // List gpn records — the value is the array of records itself.
    gpns, err := client.Gpn(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range gpns.([]any) {
        fmt.Println(item)
    }
}
```
...
gpn, err := client.Gpn(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(gpn) // the returned mock data
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *CelestrakGpDataSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### CelestrakGpDataSDK methods
...
| `Gpn` | `(data map[string]any) CelestrakGpDataEntity` | Create a Gpn entity instance. |

### Entity interface (CelestrakGpDataEntity)

All entities implement the `CelestrakGpDataEntity` interface.
...
slice):
    ...
    gpn, err := client.Gpn(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // gpn is the returned record
...
API path: `/NORAD/elements/gp.php`
...
Create an instance: `gpn := client.Gpn(nil)`
...
fmt.Println(gpns) // the array of records
```
...
```
github.com/voxgig-sdk/celestrak-gp-data-sdk/go/
├── celestrak-gp-data.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/celestrak-gp-data-sdk/go`) re-exports everything needed
...
// gpn.Data() now returns the gpn data from the last list
// gpn.Match() returns the last match criteria
```
