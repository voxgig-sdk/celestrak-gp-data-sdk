...
// CelestrakGpData SDK pipeline test
//
...
{
    private static function utility(): CelestrakGpDataUtility
    {
        return new CelestrakGpDataUtility();
    }
    ...
    private static function ctx(array $over = []): CelestrakGpDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CelestrakGpDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CelestrakGpDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CelestrakGpDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CelestrakGpDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CelestrakGpDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CelestrakGpDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CelestrakGpDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CelestrakGpDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CelestrakGpDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CelestrakGpDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CelestrakGpDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CelestrakGpDataResponse([]),
            'result' => new CelestrakGpDataResult([]),
        ]);
        [, $err] = CelestrakGpDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CelestrakGpDataSpec([]),
            'response' => null,
            'result' => new CelestrakGpDataResult([]),
        ]);
        [, $err] = CelestrakGpDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CelestrakGpDataSpec(['step' => 's']),
            'response' => new CelestrakGpDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CelestrakGpDataResult([]),
        ]);
        [, $err] = CelestrakGpDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CelestrakGpDataSpec(['step' => 's']),
            'response' => new CelestrakGpDataResponse(self::resp(200, ['v' => 1])),
            'result' => new CelestrakGpDataResult([]),
        ]);
        [, $err] = CelestrakGpDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CelestrakGpDataSpec(['step' => 's']),
            'response' => new CelestrakGpDataResponse(self::resp(200, ['v' => 2])),
            'result' => new CelestrakGpDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CelestrakGpDataMakeResponse::call($ctx);
    ...
    {
        $preset = new CelestrakGpDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CelestrakGpDataSpec([]),
            'response' => new CelestrakGpDataResponse([]),
            'result' => new CelestrakGpDataResult([]),
        ...
        [$got, $err] = CelestrakGpDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CelestrakGpDataResult([])]);
        [, $err] = CelestrakGpDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CelestrakGpDataSpec([]), 'result' => null]);
        [, $err] = CelestrakGpDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CelestrakGpDataSpec(['step' => 's']),
            'result' => new CelestrakGpDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CelestrakGpDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CelestrakGpDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CelestrakGpDataSpec(['step' => 's']),
            'result' => new CelestrakGpDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CelestrakGpDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CelestrakGpDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CelestrakGpDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CelestrakGpDataSpec([]),
            'result' => new CelestrakGpDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CelestrakGpDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CelestrakGpDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CelestrakGpDataError('boom', 'boom');
        $utility->fetcher = function (CelestrakGpDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CelestrakGpDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CelestrakGpDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CelestrakGpDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CelestrakGpDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CelestrakGpDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CelestrakGpDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CelestrakGpDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CelestrakGpDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CelestrakGpDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CelestrakGpDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CelestrakGpDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CelestrakGpDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CelestrakGpDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CelestrakGpDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CelestrakGpDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CelestrakGpDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CelestrakGpDataResult(['ok' => false])]);
        $this->expectException(CelestrakGpDataError::class);
        CelestrakGpDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CelestrakGpDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CelestrakGpDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CelestrakGpDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CelestrakGpDataBaseFeature();
        $b = new CelestrakGpDataBaseFeature();
        CelestrakGpDataFeatureAdd::call($ctx, $a);
        CelestrakGpDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CelestrakGpDataBaseFeature
    {
        $f = new CelestrakGpDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CelestrakGpDataFeatureAdd::call($ctx, self::named_feature('a'));
        CelestrakGpDataFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CelestrakGpDataFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CelestrakGpDataFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CelestrakGpDataFeatureAdd::call($ctx, $replace);
        ...
        CelestrakGpDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CelestrakGpDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CelestrakGpDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CelestrakGpDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CelestrakGpDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CelestrakGpDataResponse(['headers' => null]),
            'result' => new CelestrakGpDataResult([]),
        ]);
        CelestrakGpDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CelestrakGpDataResultBody::call($ctx);
