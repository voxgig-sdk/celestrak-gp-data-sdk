...
// CelestrakGpData SDK feature test
//
...
class FtCtrl extends CelestrakGpDataControl
{
...
class FtClient
{
    ...
    private CelestrakGpDataUtility $utility;

    public function __construct(CelestrakGpDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): CelestrakGpDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CelestrakGpDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CelestrakGpDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CelestrakGpDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CelestrakGpDataConfig::make_feature($name);
    ...
    private static function buildUrl(CelestrakGpDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CelestrakGpDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CelestrakGpDataResponse($fetched);
        }
    ...
    private function fail(CelestrakGpDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CelestrakGpDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
