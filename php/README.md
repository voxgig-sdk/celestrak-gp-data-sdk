# CelestrakGpData PHP SDK



The PHP SDK for the CelestrakGpData API — an entity-oriented client using PHP conventions.
...
$gpn = $client->Gpn()->list();
print_r(array_map(fn($item) => $item->data_get(), $gpn));
```
...
```
php/
├── celestrakgpdata_sdk.php          -- Main SDK class
...
The main class (`celestrakgpdata_sdk.php`) exports the SDK class
