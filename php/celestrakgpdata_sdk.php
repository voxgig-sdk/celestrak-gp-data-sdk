...
class CelestrakGpDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CelestrakGpDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CelestrakGpDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CelestrakGpDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CelestrakGpDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CelestrakGpDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CelestrakGpDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CelestrakGpDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CelestrakGpDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CelestrakGpDataError($op . "_allow",
                "CelestrakGpDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CelestrakGpDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CelestrakGpDataError("graphql_error",
                "CelestrakGpDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Gpn($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_gpn === null) {
                $this->_gpn = new GpnEntity($this, null);
            }
            return $this->_gpn;
        }
        return new GpnEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CelestrakGpDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
