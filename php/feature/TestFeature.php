...
// CelestrakGpData SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CelestrakGpDataTestFeature extends CelestrakGpDataBaseFeature
{
    ...
    public function init(CelestrakGpDataContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CelestrakGpDataContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CelestrakGpDataParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CelestrakGpDataContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CelestrakGpDataContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CelestrakGpDataParam::call($ctx, $k);
