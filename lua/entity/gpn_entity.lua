...
local GpnEntity = {}
GpnEntity.__index = GpnEntity


function GpnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GpnEntity)
  self._name = "gpn"
  self._client = client
...
function GpnEntity:get_name()
  return self._name
end


function GpnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GpnEntity.new(self._client, opts)
end
...
function GpnEntity:mark_deleted()
  self._deleted = true
end


function GpnEntity:deleted()
  return true == self._deleted
end


function GpnEntity:data_set(args)
  if args ~= nil then
...
function GpnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GpnEntity:match_set(args)
  if args ~= nil then
...
function GpnEntity:stream(action, args, callopts)
...
function GpnEntity:_run_op(ctx, post_done)
  local utility = self._utility
