# celestrak-gp-data-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CelestrakGpData SDK as
two agent tools — `celestrak-gp-data_list` and `celestrak-gp-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/celestrak-gp-data-mcp)
make build

# 2. Provide credentials via the environment
export CELESTRAK_GP_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user celestrak-gp-data \
  -- /absolute/path/to/celestrak-gp-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./celestrak-gp-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// celestrak-gp-data_list: first page of records
{ "entity": "gpn" }
{ "entity": "gpn", "query": { } }

// celestrak-gp-data_load: one record by id
{ "entity": "gpn", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/celestrak-gp-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user celestrak-gp-data \
     -- "$PWD"/dist/*/celestrak-gp-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `celestrak-gp-data_list` and `celestrak-gp-data_load` tools now appear
   in new sessions. Ask the agent to *"list gpn using celestrak-gp-data"*
   and it calls `celestrak-gp-data_list` with `{"entity":"gpn"}`.
...
```sh
export CELESTRAK_GP_DATA_APIKEY=sk_live_xxx            # API key
export CELESTRAK_GP_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `celestrak-gp-data_list` tool
...
### Call the `celestrak-gp-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `celestrak-gp-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `celestrak-gp-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CELESTRAK_GP_DATA_APIKEY` | API key sent with every request. |
| `CELESTRAK_GP_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./celestrak-gp-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"celestrak-gp-data_load","arguments":{"entity":"gpn","query":{"id":1}}}}'
```
