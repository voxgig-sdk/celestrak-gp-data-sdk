...
CelesTrak provides access to General Perturbations (GP) orbital data, including Two-Line Element Sets (TLEs) and newer formats like XML and JSON.

Learn more about CelesTrak GP Data at [celestrak.org/NORAD/documentation/gp-data-formats.php](https://celestrak.org/NORAD/documentation/gp-data-formats.php).

This is an unofficial SDK for the CelesTrak GP Data public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Gpn — that you
...
const items = await client.Gpn().list()
```
...
```go
client := sdk.Test()
result, err := client.Gpn(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| TypeScript | `@voxgig-sdk/celestrak-gp-data` | publish pending — [install from git tag](https://github.com/voxgig-sdk/celestrak-gp-data-sdk/releases) |
...
| --- | --- | --- |
| **Gpn** | The Gpn entity (list). | `/NORAD/elements/gp.php` |
