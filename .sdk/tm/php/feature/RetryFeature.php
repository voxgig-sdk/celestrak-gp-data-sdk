...
// CelestrakGpData SDK retry feature
...
class CelestrakGpDataRetryFeature extends CelestrakGpDataBaseFeature
{
    ...
    public function init(CelestrakGpDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CelestrakGpDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CelestrakGpDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CelestrakGpDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
