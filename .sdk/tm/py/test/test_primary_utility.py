...
from projectname_sdk import CelestrakGpDataSDK
from projectname_sdk.core.spec import CelestrakGpDataSpec
from projectname_sdk.core.result import CelestrakGpDataResult
from projectname_sdk.core.response import CelestrakGpDataResponse
from projectname_sdk.core.operation import CelestrakGpDataOperation
from projectname_sdk.core.error import CelestrakGpDataError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import CelestrakGpDataBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return CelestrakGpDataError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except CelestrakGpDataError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = CelestrakGpDataResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = CelestrakGpDataSDK.test(None, None)
        ...
        class TestInitFeature(CelestrakGpDataBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(CelestrakGpDataBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = CelestrakGpDataSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = CelestrakGpDataSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = CelestrakGpDataSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = CelestrakGpDataSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = CelestrakGpDataOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = CelestrakGpDataSDK.test(None, setup_opts)
