...
from projectname_sdk.core.control import CelestrakGpDataControl
...
class CelestrakGpDataContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = CelestrakGpDataControl()
        ...
        if isinstance(sp, CelestrakGpDataSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CelestrakGpDataResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CelestrakGpDataResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return CelestrakGpDataOperation({})
    ...
    def make_error(self, code, msg):
        return CelestrakGpDataError(code, msg, self)
