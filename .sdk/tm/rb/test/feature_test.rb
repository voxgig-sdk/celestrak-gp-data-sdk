...
module CelestrakGpDataFeatureHarness
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || CelestrakGpDataFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless CelestrakGpDataFeatureHarness.has_feature?(name)
        f = CelestrakGpDataFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= CelestrakGpDataFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(CelestrakGpDataError)
        ...
        ctx.response = fetched.is_a?(Hash) ? CelestrakGpDataResponse.new(fetched) : nil
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = CelestrakGpDataResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = CelestrakGpDataFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, CelestrakGpDataError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, CelestrakGpDataError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
